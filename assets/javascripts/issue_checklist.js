...
Redmine.IssueChecklist = Class.create({
  initialize: function (element, input, key, quantity, button) {
  ...
  readChecklist: function (event) {
    this.addChecklistItem(this.input.value, this.key.value, this.quantity.value);
    this.input.value = '';
    this.key.value = '';
    this.quantity.value = '';
    Event.stop(event);
  ...
  addChecklistItem: function (сhecklistItem, key, quantity, isDone) {
    ...
    var label = key + ' - ' + checklistItem + ' - ' + quantity; 
